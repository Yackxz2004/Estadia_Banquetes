...
const Periqueras = () => {
  ...
  const fetchItems = async () => {
    try {
      const response = await api.get('/api/inventory/periqueras/', { params: { search: searchTerm } });
      setItems(response.data);
    } catch (error) {
      console.error('Error fetching periqueras items:', error);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await api.put(`/api/inventory/periqueras/${currentItemId}/`, data);
      } else {
        await api.post('/api/inventory/periqueras/', data);
      }
  ...
  const handleEdit = (item) => {
    ...
    setFormModal({ isOpen: true, title: 'Editar Periquera' });
  };
  ...
  const openCreateModal = () => {
    ...
    setFormData({ producto: '', descripcion: '', cantidad: '', bodega: '' });
    setFormModal({ isOpen: true, title: 'Nuevo Elemento de Periquera' });
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este elemento?')) {
      try {
        await api.delete(`/api/inventory/periqueras/${id}/`);
        fetchItems();
      } catch (error) {
  ...
  const handleMantenimientoAction = async (actionType) => {
    ...
    try {
      if (actionType === 'enviar') {
        await enviarAMantenimiento('periqueras', item.id, cantidadNum);
      } else if (actionType === 'reintegrar') {
        await reintegrarDeMantenimiento('periqueras', item.id, cantidadNum);
      }
      fetchItems();
  ...
  return (
    <div className="periqueras-container">
      <div className="periqueras-header">
        <h1 className="periqueras-title">Gestión de Periqueras</h1>
      </div>
      ...
      <div className="table-responsive">
        <table className="periqueras-table">
          <thead>
            <tr>
              <th>Producto</th>
              <th>Descripción</th>
            ...
            {items.length > 0 ? (
              ...
              <tr>
                <td colSpan="6" className="no-items">
                  No se encontraron elementos de periqueras.
                </td>
              </tr>
            )}
          </tbody>
      ...
      {formModal.isOpen && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <form onSubmit={handleSubmit} className="periqueras-form-modal">
