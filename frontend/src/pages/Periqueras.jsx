  ...
  const handleMantenimientoAction = async (actionType) => {
    ...
    try {
      if (actionType === 'enviar') {
        await enviarAMantenimiento('periqueras', item.id, parseInt(cantidad));
      } else if (actionType === 'reintegrar') {
        await reintegrarDeMantenimiento('periqueras', item.id, parseInt(cantidad));
      }
      fetchItems();
