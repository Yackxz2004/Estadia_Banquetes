...
import { FiPlus, FiEdit2, FiTrash2, FiTool, FiArrowLeft, FiX } from 'react-icons/fi';
import { getBodegas, enviarAMantenimiento, reintegrarDeMantenimiento } from '../api/inventory';
import '../styles/Manteleria.css';
import api from '../api';
...
const Manteleria = () => {
  ...
  const fetchItems = async () => {
    try {
      const response = await api.get('/api/inventory/mantelerias/', { params: { search: searchTerm } });
      setItems(response.data);
    } catch (error) {
      console.error('Error fetching manteleria items:', error);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await api.put(`/api/inventory/mantelerias/${currentItemId}/`, data);
      } else {
        await api.post('/api/inventory/mantelerias/', data);
      }
  ...
  const handleEdit = (item) => {
    ...
    setFormModal({ isOpen: true, title: 'Editar Mantelería' });
  };
  ...
  const openCreateModal = () => {
    ...
    setFormData({ producto: '', descripcion: '', cantidad: '', bodega: '' });
    setFormModal({ isOpen: true, title: 'Nuevo Elemento de Mantelería' });
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este elemento?')) {
      try {
        await api.delete(`/api/inventory/mantelerias/${id}/`);
        fetchItems();
      } catch (error) {
  ...
  const handleMantenimientoAction = async (actionType) => {
    ...
    try {
      if (actionType === 'enviar') {
        await enviarAMantenimiento('mantelerias', item.id, cantidadNum);
      } else if (actionType === 'reintegrar') {
        await reintegrarDeMantenimiento('mantelerias', item.id, cantidadNum);
      }
      fetchItems();
  ...
  return (
    <div className="manteleria-container">
      <div className="manteleria-header">
        <h1 className="manteleria-title">Gestión de Mantelería</h1>
      ...
      <div className="table-responsive">
        <table className="manteleria-table">
          <thead>
            <tr>
              <th>Producto</th>
              <th>Descripción</th>
            ...
            {items.length > 0 ? (
              ...
              <tr>
                <td colSpan="6" className="no-items">
                  No se encontraron elementos de mantelería.
                </td>
              </tr>
            )}
          </tbody>
      ...
      {formModal.isOpen && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <form onSubmit={handleSubmit} className="manteleria-form-modal">
