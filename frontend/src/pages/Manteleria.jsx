  ...
  const handleMantenimientoAction = async (actionType) => {
    ...
    try {
      if (actionType === 'enviar') {
        await enviarAMantenimiento('mantelerias', item.id, parseInt(cantidad));
      } else if (actionType === 'reintegrar') {
        await reintegrarDeMantenimiento('mantelerias', item.id, parseInt(cantidad));
      }
      fetchItems();
