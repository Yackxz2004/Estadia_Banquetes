...
const Extras = () => {
  ...
  const fetchItems = async () => {
    try {
      const response = await api.get('/api/inventory/extras/', { params: { search: searchTerm } });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await api.put(`/api/inventory/extras/${currentItemId}/`, data);
      } else {
        await api.post('/api/inventory/extras/', data);
      }
  ...
  const handleEdit = (item) => {
    ...
    setFormModal({ isOpen: true, title: 'Editar Extra' });
  };
  ...
  const openCreateModal = () => {
    ...
    setFormData({ producto: '', descripcion: '', cantidad: '', bodega: '' });
    setFormModal({ isOpen: true, title: 'Nuevo Elemento Extra' });
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este elemento?')) {
      try {
        await api.delete(`/api/inventory/extras/${id}/`);
        fetchItems();
      } catch (error) {
  ...
  const handleMantenimientoAction = async (actionType) => {
    ...
    try {
      if (actionType === 'enviar') {
        await enviarAMantenimiento('extras', item.id, cantidadNum);
      } else if (actionType === 'reintegrar') {
        await reintegrarDeMantenimiento('extras', item.id, cantidadNum);
      }
      fetchItems();
  ...
  return (
    <div className="extras-container">
      <div className="extras-header">
        <h1 className="extras-title">Gestión de Extras</h1>
      </div>
            ...
            {items.length > 0 ? (
              ...
              <tr>
                <td colSpan="6" className="no-items">
                  No se encontraron elementos extras.
                </td>
              </tr>
            )}
          </tbody>
      ...
      {formModal.isOpen && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <form onSubmit={handleSubmit} className="extras-form-modal">
