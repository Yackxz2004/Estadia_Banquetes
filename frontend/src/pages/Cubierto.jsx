...
const Cubierto = () => {
  ...
  const fetchItems = async () => {
    try {
      const response = await api.get('/api/inventory/cubiertos/', { params: { search: searchTerm } });
      setItems(response.data);
    } catch (error) {
      console.error('Error fetching cubierto items:', error);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await api.put(`/api/inventory/cubiertos/${currentItemId}/`, data);
      } else {
        await api.post('/api/inventory/cubiertos/', data);
      }
  ...
  const handleEdit = (item) => {
    ...
    setFormModal({ isOpen: true, title: 'Editar Cubierto' });
  };
  ...
  const openCreateModal = () => {
    ...
    setFormData({ producto: '', descripcion: '', cantidad: '', bodega: '' });
    setFormModal({ isOpen: true, title: 'Nuevo Elemento de Cubierto' });
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este elemento?')) {
      try {
        await api.delete(`/api/inventory/cubiertos/${id}/`);
        fetchItems();
      } catch (error) {
  ...
  const handleMantenimientoAction = async (actionType) => {
    ...
    try {
      if (actionType === 'enviar') {
        await enviarAMantenimiento('cubiertos', item.id, cantidadNum);
      } else if (actionType === 'reintegrar') {
        await reintegrarDeMantenimiento('cubiertos', item.id, cantidadNum);
      }
      fetchItems();
  ...
  return (
    <div className="cubierto-container">
      <div className="cubierto-header">
        <h1 className="cubierto-title">Gestión de Cubiertos</h1>
      </div>
      ...
      <div className="table-responsive">
        <table className="cubierto-table">
          <thead>
            <tr>
              <th>Producto</th>
              <th>Descripción</th>
            ...
            {items.length > 0 ? (
              ...
              <tr>
                <td colSpan="6" className="no-items">
                  No se encontraron elementos de cubierto.
                </td>
              </tr>
            )}
          </tbody>
      ...
      {formModal.isOpen && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <form onSubmit={handleSubmit} className="cubierto-form-modal">
