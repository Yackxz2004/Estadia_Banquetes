  ...
  const handleMantenimientoAction = async (actionType) => {
    ...
    try {
      if (actionType === 'enviar') {
        await enviarAMantenimiento('cubiertos', item.id, parseInt(cantidad));
      } else if (actionType === 'reintegrar') {
        await reintegrarDeMantenimiento('cubiertos', item.id, parseInt(cantidad));
      }
      fetchItems();
