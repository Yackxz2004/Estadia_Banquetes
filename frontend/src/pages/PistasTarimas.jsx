...
import '../styles/PistasTarimas.css';
import api from '../api';
...
const PistasTarimas = () => {
  ...
  const fetchItems = async () => {
    try {
      const response = await api.get('/api/inventory/pistas-tarimas/', { params: { search: searchTerm } });
      setItems(response.data);
    } catch (error) {
      console.error('Error fetching pistas y tarimas items:', error);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await api.put(`/api/inventory/pistas-tarimas/${currentItemId}/`, data);
      } else {
        await api.post('/api/inventory/pistas-tarimas/', data);
      }
  ...
  const handleEdit = (item) => {
    ...
    setFormModal({ isOpen: true, title: 'Editar Pista/Tarima' });
  };
  ...
  const openCreateModal = () => {
    ...
    setFormData({ producto: '', descripcion: '', cantidad: '', bodega: '' });
    setFormModal({ isOpen: true, title: 'Nuevo Elemento de Pista/Tarima' });
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este elemento?')) {
      try {
        await api.delete(`/api/inventory/pistas-tarimas/${id}/`);
        fetchItems();
      } catch (error) {
  ...
  const handleMantenimientoAction = async (actionType) => {
    ...
    try {
      if (actionType === 'enviar') {
        await enviarAMantenimiento('pistas-tarimas', item.id, cantidadNum);
      } else if (actionType === 'reintegrar') {
        await reintegrarDeMantenimiento('pistas-tarimas', item.id, cantidadNum);
      }
      fetchItems();
  ...
  return (
    <div className="pistastarimas-container">
      <div className="pistastarimas-header">
        <h1 className="pistastarimas-title">Gestión de Pistas y Tarimas</h1>
      </div>
      ...
      <div className="search-container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <input 
          ...
          placeholder="Buscar pistas y tarimas..." 
          value={searchTerm} 
      ...
      <div className="table-responsive">
        <table className="pistastarimas-table">
          <thead>
            <tr>
              <th>Producto</th>
              <th>Descripción</th>
            ...
            {items.length > 0 ? (
              ...
              <tr>
                <td colSpan="6" className="no-items">
                  No se encontraron elementos de pistas y tarimas.
                </td>
              </tr>
            )}
          </tbody>
      ...
      {formModal.isOpen && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <form onSubmit={handleSubmit} className="pistastarimas-form-modal">
