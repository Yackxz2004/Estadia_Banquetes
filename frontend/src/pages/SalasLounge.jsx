...
const SalasLounge = () => {
  ...
  const fetchItems = async () => {
    try {
      const response = await api.get('/api/inventory/salas-lounge/', { params: { search: searchTerm } });
      setItems(response.data);
    } catch (error) {
      console.error('Error fetching salas lounge items:', error);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await api.put(`/api/inventory/salas-lounge/${currentItemId}/`, data);
      } else {
        await api.post('/api/inventory/salas-lounge/', data);
      }
  ...
  const handleEdit = (item) => {
    ...
    setFormModal({ isOpen: true, title: 'Editar Sala Lounge' });
  };
  ...
  const openCreateModal = () => {
    ...
    setFormData({ producto: '', descripcion: '', cantidad: '', bodega: '' });
    setFormModal({ isOpen: true, title: 'Nuevo Elemento de Sala Lounge' });
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este elemento?')) {
      try {
        await api.delete(`/api/inventory/salas-lounge/${id}/`);
        fetchItems();
      } catch (error) {
  ...
  const handleMantenimientoAction = async (actionType) => {
    ...
    try {
      if (actionType === 'enviar') {
        await enviarAMantenimiento('salas-lounge', item.id, cantidadNum);
      } else if (actionType === 'reintegrar') {
        await reintegrarDeMantenimiento('salas-lounge', item.id, cantidadNum);
      }
      fetchItems();
  ...
  return (
    <div className="salaslounge-container">
      <div className="salaslounge-header">
        <h1 className="salaslounge-title">Gestión de Salas Lounge</h1>
      </div>
      ...
      <div className="search-container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <input 
          ...
          placeholder="Buscar salas lounge..." 
          value={searchTerm} 
      ...
      <div className="table-responsive">
        <table className="salaslounge-table">
          <thead>
            <tr>
              <th>Producto</th>
              <th>Descripción</th>
            ...
            {items.length > 0 ? (
              ...
              <tr>
                <td colSpan="6" className="no-items">
                  No se encontraron elementos de salas lounge.
                </td>
              </tr>
            )}
          </tbody>
      ...
      {formModal.isOpen && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <form onSubmit={handleSubmit} className="salaslounge-form-modal">
