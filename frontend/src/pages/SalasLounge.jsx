  ...
  const handleMantenimientoAction = async (actionType) => {
    ...
    try {
      if (actionType === 'enviar') {
        await enviarAMantenimiento('salas-lounge', item.id, parseInt(cantidad));
      } else if (actionType === 'reintegrar') {
        await reintegrarDeMantenimiento('salas-lounge', item.id, parseInt(cantidad));
      }
      fetchItems();
