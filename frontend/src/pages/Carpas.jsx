...
const Carpas = () => {
  ...
  const fetchItems = async () => {
    try {
      const response = await api.get('/api/inventory/carpas/', { params: { search: searchTerm } });
      setItems(response.data);
    } catch (error) {
      console.error('Error fetching carpas items:', error);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await api.put(`/api/inventory/carpas/${currentItemId}/`, data);
      } else {
        await api.post('/api/inventory/carpas/', data);
      }
  ...
  const handleEdit = (item) => {
    ...
    setFormModal({ isOpen: true, title: 'Editar Carpa' });
  };
  ...
  const openCreateModal = () => {
    ...
    setFormData({ producto: '', descripcion: '', cantidad: '', bodega: '' });
    setFormModal({ isOpen: true, title: 'Nuevo Elemento de Carpa' });
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este elemento?')) {
      try {
        await api.delete(`/api/inventory/carpas/${id}/`);
        fetchItems();
      } catch (error) {
  ...
  const handleMantenimientoAction = async (actionType) => {
    ...
    try {
      if (actionType === 'enviar') {
        await enviarAMantenimiento('carpas', item.id, cantidadNum);
      } else if (actionType === 'reintegrar') {
        await reintegrarDeMantenimiento('carpas', item.id, cantidadNum);
      }
      fetchItems();
  ...
  return (
    <div className="carpas-container">
      <div className="carpas-header">
        <h1 className="carpas-title">Gestión de Carpas</h1>
      </div>
            ...
            {items.length > 0 ? (
              ...
              <tr>
                <td colSpan="6" className="no-items">
                  No se encontraron elementos de carpas.
                </td>
              </tr>
            )}
          </tbody>
      ...
      {formModal.isOpen && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <form onSubmit={handleSubmit} className="carpas-form-modal">
